...
will ensure consistent results with the lowest uncertainty possible.
	...
	\item Adjust mirror so you can see into the flask
     ...
     \item Prepare sample on ARIA
		...
		\item Liquid/Gases Sample
			\begin{itemize}
			\item Place syringe securely into syringe holder on ARIA, making sure the tip of the syringe 
			needle is aligned with the hole in the furnace lid
			\end{itemize}
			
		\item Solid Sample
			\begin{itemize}
			\item Carefully insert weigh boat onto holder on ARIA servo, making sure that it is secure
			\item As securely and vertically as possible, place the funnel on the ring stand arm of ARIA
			\end{itemize}
			
		%\end{itemize}
			
	\item If it is the first run of the day, ensure that ARIA is in the lowered position. 
		\begin{itemize} 
		\item If it is not in the lowered position, manually lower it until
			it is touching the plate. 
		\item Plug in the ARIA power source and allow it to reach stand-by position. The LED lights 
		will be lit, indicating it is in stand-by mode.
		\end{itemize}
	
		\item If it is not the first run of the day, ensure that ARIA is in stand-by mode and the arm is in the raised 
		stand-by position. The LED lights will be lit, indicating it is in stand-by mode.
	
			
	\item Remove gloves for touching any non-hazardous surfaces
	
	\item Place lid on pressure vessel and secure in place with the clamps.
		\begin{itemize}
		\item Hand-tighten clamps gently all the way around, and then tighten again in a 
		"criss-cross" pattern in the numbered order indicated on the clamps (hand tight is all that is necessary, 
		but we talked about potentially using a torque wrench) This involves tightening two directly across from 
		each other, then skipping one set to do the next one, and then repeating until all have been tightened.
		\end{itemize}
		
	\item Note the absolute pressure in the lab on the barometer mounted on the east wall by the fume hood 
	and write down the pressure in the lab notebook
	...
	\item Pressurize vessel
		...
		\item Adjust pressure in vessel using air cylinder valves until the pressure in the vessel and the 
		absolute pressure of the lab add up to 1 atm (760 torr)
		\end{itemize} 
		
	\item Ensure that there are no leaks
	    \begin{itemize}
	    \item First check by running your fingers all around the lid. If you feel air coming out, shut off the air flow, 
		allow the vessel to depressurize fully, and remove the lid to see what the problem is. Ensure that the ARIA 
		and the cords are placed in such a way that they would not be in the way of the lid sealing all the way. Also 
		check to make sure that the o-ring around the vessel is clean (no dust, debris etc.) so that proper sealing is possible. 
		When you have located and solved the problem, go through the steps to pressurize again and then test it again.
	    \item Second, do an additional pressure check by shutting off the air flow and watching the pressure readings. 
		If the pressure is staying about constant over time, you can know that the vessel is properly sealed.
	    \item Turn air flow back on
	    \end{itemize}

	\item Turn off lights and ensure the lab is sufficiently dark to see any flame from the mirror on top of the furnace in the vessel.
    ...
    \item After the experiment ends, terminate data collection
		...
		\item Wait the specified time, about 10-20 minutes to allow for the pressure vessel to be purged of the combustion products
	
	\item Turn off the inlet air flow and wait for pressure vessel to FULLY depressurize (outflow rotameter reads zero)
