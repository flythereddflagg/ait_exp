...
\end{figure}
    ...
    \item Plug the air cylinder solenoid valve power into the 120V outlet on the hood
...
\end{figure}
    ...
    \item Twist both of the ARIA lead screws by hand such that the mounting 
        ...
        horizontal stepper motor (this is the shutdown position) % SEE TRAINING
...
\newpage		
    ...
    \item Extract, save and appropriately rename the video data between 
            experiments
	\item After the purge time has ended, turn off the inlet air flow and wait 
        for pressure vessel to \textit{fully} depressurize (outflow rotometer reads zero)
	\item Remove the pressure vessel lid 
		\begin{itemize}
		\item Loosen the clamps using a regular wrench instead of a 
            torque wrench
