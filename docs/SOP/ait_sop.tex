...
\end{figure}
    ...
    \item Check the temperature of the furnace to ensure safe handling for 
        ...
        \item If the temperature does not read close to ambient temperature, do 
            do not begin experiments. 
        \end{itemize}
    ...
    \item Test the placement of the ARIA by using an empty sample of the type 
        you will be working with today
         \begin{itemize}
         \item For liquids/gases, place the empty syringe in the ARIA, lined up 
            over the flask, and run the Liquid program.
         \item For solids, place the ring stand in the ARIA, then place the 
            funnel in the ring stand, then run the solid program
         \item If the syringe or the funnel do not go directly into the flask, 
            adjust the placement and test until the proper positioning is found
         \end{itemize}
...
will ensure consistent results with the lowest uncertainty possible.
    ...
    \item Measure out sample
        ...
        \item Liquids
            ...
            \item Draw sample amount into a right-angle syringe
                \begin{itemize}
                \item This should not exceed about 300 microliters
                \end{itemize}
            \end{itemize}
	...
	\item Adjust mirror so you can see into the flask
    
	\item If it is the first run of the day, ensure that ARIA is in the lowered 
        position. 
		\begin{itemize} 
		\item If it is not in the lowered position, manually lower it until
			it is touching the plate. 
		\item Plug in the ARIA power source and allow it to reach stand-by 
            position. The LED lights will be lit, indicating it is in stand-by 
            mode.
		\end{itemize}
	
		\item If it is not the first run of the day, ensure that ARIA is in 
            stand-by mode and the arm is in the raised stand-by position. The 
            LED lights will be lit, indicating it is in stand-by mode.
	
    
     \item Prepare sample on ARIA
		\begin{itemize}
		\item Liquid/Gases Sample
			\begin{itemize}
			\item Place syringe securely into syringe holder on ARIA, making 
                sure the tip of the syringe 
                needle is aligned with the hole in the furnace lid
			\end{itemize}
			
		\item Solid Sample
			\begin{itemize}
			\item Carefully insert weigh boat onto holder on ARIA servo, making 
                sure that it is secure
			\item As securely and vertically as possible, place the funnel on 
                the ring stand arm of ARIA
			\end{itemize}
	...
	\item Remove gloves for touching any non-hazardous surfaces
	
	\item Place lid on pressure vessel and secure in place with the clamps. Make
        sure to line up the marks on the lid with the marks on the vessel.
		\begin{itemize}
		\item Hand-tighten clamps gently all the way around, and then tighten 
            again in a "criss-cross" pattern in the numbered order indicated on the 
            clamps. 
            %(hand tight is all that is necessary but we talked about potentially using a torque wrench).
            This involves tightening two directly across from each other, then 
            skipping one set to do the next one, and then repeating until all have been tightened.
        \item After the "criss-cross" pattern tightening is completed, double 
        check that all clamps are tightened by going all the way around once more
        tightening fully
		\end{itemize}
		
	\item Note the absolute pressure in the lab on the barometer mounted on the 
    east wall by the fume hood and write down the pressure in the lab notebook
		
	\item Pressurize vessel
		\begin{itemize}
		\item Open the pressure transducer measurements by going to the TADA 
            firmware Arduino file and clicking 
			"Serial Monitor" in the top right-hand corner.
        \item Ensure the ball valve after the regulators on the compressed air 
            tank is closed
        %\item Place and secure the safety chain\cable 
        \item Fully open the rotometer on the exhaust of the pressure vessel
        \item Slowly open the valve on the compressed air tank. Once pressurized,
            open the valve fully, then close one quarter turn
        \item Open the ball valve fully
            \begin{itemize}
            \item Note: If a loud, high pitched squeal is heard, immediately 
                close the ball valve and allow the vessel to fully depressurize,
                it is a sign of a leak
            \end{itemize}
        \item Slowly close the rotometer until the indicator shows just above the 
            lowest mark
		\item Adjust pressure in vessel using air cylinder valves until the    
            pressure in the vessel and the absolute pressure of the lab add up 
            to 1 atm (760 torr)
		\end{itemize} 
		
	\item Ensure that there are no leaks
	    \begin{itemize}
	    \item First check by running your fingers all around the lid. If you 
            feel air coming out, shut off the air flow, allow the vessel to 
            depressurize fully, and remove the lid to see what the problem is. 
            Ensure that the ARIA and the cords are placed in such a way that 
            they would not be in the way of the lid sealing all the way. Also 
            check to make sure that the O-ring around the vessel is clean (no 
            dust, debris etc.) so that proper sealing is possible. When you have
            located and solved the problem, go through the steps to pressurize 
            again and then test it again.
	    \item Second, do an additional pressure check by shutting off the air 
            flow and watching the pressure readings. If the pressure is staying 
            about constant over time, you can know that the vessel is properly 
            sealed.
	    \item Turn air flow back on
	    \end{itemize}

	\item Turn off lights and ensure the lab is sufficiently dark to see any 
        flame from the mirror on top of the furnace in the vessel.
    ...
    \item Record pertinent data and observations in the lab book and the 
        ...
        \item The following data must be present on the same row, in the 
            ...
            \item The total barometric pressure, the ambient room pressure and 
                the pressure vessel gauge pressure at the time of the experiment
                (in mmHg)
            \end{itemize}
    ...
    \item Wait the specified time, about 20 minutes to allow for the pressure 
	...
	\item After the purge time as ended, turn off the inlet air flow and wait 
        for pressure vessel to FULLY depressurize (outflow rotameter reads zero)
...
\subsection{Shutdown}
    ...
    \item A hot furnace may be left in the pressure vessel without waiting for 
        it to cool
...
\section{Emergency Shutdown} \label{sec:e_shtdn}
    ...
    \item In the event of an emergency do the following:
        ...
        \begin{itemize}
        \item Turn off the air at the compressed air cylinder
        \item Power off the furnace
        \item Fully open the rotameter exhaust
