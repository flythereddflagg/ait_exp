...
\end{figure}
    ...
    \item Twist both of the ARIA lead screws by hand such that the mounting 
        ...
        the shutdown position) % SEE TRAINING
...
\end{figure}
    ...
    \item Test the placement of the ARIA using a blank sample
         ...
         \item For liquids/gases, place the empty syringe in the ARIA, lined up 
            over the flask, and run the Liquid program (Press the blue Liquid 
            button)
...
\end{figure}
    ...
    \item Remove the snorkel from inside the vessel and place above the rupture 
        disk (see Figure \ref{fig:rupture_hood}) then immediately place lid on 
...
\end{figure}
    ...
    \item Pressurize the vessel
        ...
        \item Fully open the rotometer on the exhaust of the pressure vessel by 
            gently rotating the rotometer knob counterclockwise 
        ...
        \item Slowly close the rotometer (rotate clockwise) until the air flow 
            reads 0.4 SCFM (the lowest graduation on the rotometer)
        \item Adjust pressure in vessel using low pressure regulator until the    
            ...
            indicating that the pressure in the vessel is 1 atm (760 Torr)
    ...
    \item Wait about 20 minutes after ignition to allow for the pressure 
        vessel to be purged of the combustion products. If there was no combustion,
		the only wait 10 minutes for the pressure vessel to purge 
