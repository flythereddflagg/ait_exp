...
\end{figure}
    ...
    \item Ensure the furnace thermocouples are connected to their corresponding connectors inside 
		the vessel (see Figure \ref{fig:Thermocouples}) %%Connectors? Picture?
...
\end{figure}		
    ...
    \item Connect the TADA to the lab computer via the USB cable mounted 
        under the edge of the hood (see Figure \ref{fig:TADA_USB})
...
\end{figure}
    ...
    \item Plug in the 24 volt power supply to the TADA
...
\end{figure}
    ...
    \item Ensure the three molex cables are securely plugged in to the ARIA (see Figure \ref{fig:Molex_cables})
		\begin{itemize}
		\item There are three connection points shown in Figure \ref{fig:Molex_cables}
		\item Regardless of the experiments to be performed, ensure all are plugged in and the 
...
\end{figure}
    ...
    \item Plug in the ARIA control to the 5 volt power supply and wait for 
        ...
        control will come on when the sequence is complete) (See Figure \ref{fig:5V_cable})
...
\end{figure}
	...
	\item Begin data collection 
		...
		\item A red LED in the TADA should begin blinking
    ...
    \item Watch the TADA UI for a large temperature spike for 10 minutes
        \begin{itemize}
...
\subsection{Shutdown}
    ...
    \item The following should be done before leaving the lab at the end of 
        ...
        \item Unplug TADA from the computer
...
\end{figure}
        ...
        \item Connect the thermocouple connectors to join the  
            leads from the flask to the TADA
...
\end{figure}
    ...
    \subsubsection{Connecting to the camera} 
        \begin{itemize}
        \item Firmly press and release the "info/wireless" button on the back of the camera multiple times until you see "APP \& RC" on the camera status screen
