...
will ensure consistent results with the lowest uncertainty possible.
    ...
    \item Prepare sample on ARIA
		...
		\item Liquid/Gases Sample
			begin{itemize}
			\item Place syringe into syringe holder on ARIA, making sure the tip of the syringe is 
				aligned with the hole in the furnace lid
			\end{itemize}
			
		\item Solid Sample
			\begin{itemize}
			\item Carefully insert weigh boat onto holder on ARIA, making sure that it is secure
	...
	\item If it is the first run of the day, ensure that ARIA is in the lowered position. 
		...
		\item Plug in the ARIA power source and allow it to reach stand-by position. The LED lights will be lit, indicating it is 
			in stand-by mode.
		\end{itemize}
	...
	\item Place lid on pressure vessel and secure in place with the clamps.
		\begin{itemize}
		\item Tighten in a star pattern
		\end{itemize}
		
	\item Note the absolute pressure in the lab on the barometer 
        mounted on the east wall by the fume hood and write down the pressure
        in the lab notebook 
		
	\item Pressurize vessel
		\begin{itemize}
		\item Open the pressure transducer measurements by going to the TADA firmware Arduino file and clicking 
			"Serial Monitor" in the top right-hand corner.
		\item Adjust pressure in vessel using air tank valve until the pressure in the vessel and the absolute pressure of the 
			lab add up to 1 atm (760 torr)
		\end{itemize} 
	...
	\item Begin data collection 
		begin{itemize}
		\item Use your left (ungloved) hand to press the Enter key
    ...
    \item With your LEFT (ungloved) hand, press the red button on the tablet screen to 
        start recording
